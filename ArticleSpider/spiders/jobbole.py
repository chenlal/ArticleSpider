...
class JobboleSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for post_url in post_urls:
            yield Request(parse.urljoin(response.url,post_url), self.parse_detail)
            print(post_url)
        #提取下一页并提交scrapy
    ...
    def parse_detail(self,response):
        ...
        re_match_collection = re.match(".*?(\d+).*", comment_nums)
        if re_match_collection:
            comment_nums = int(re_match_collection.group(1))
        else:
            comment_nums = 0
        ...
        tag_list = [element for element in tag_list if not element.strip().endswith("评论")]
        tags = '·'.join(tag_list)
        print(title,creator,create_date,prise_nums,collection_nums,comment_nums,tags)
        # pass
